...
### 🎷 息で、歌うようにAIとセッションしませんか？

Melody Flow は、PC のキーボードや MIDI キーボードだけでなく、新しいタイプの楽器とも連携できます。

**[UniSuling](https://github.com/u-masao/uni-suling)** （ユニスリン）は、私が開発した「息で奏でる、単音の楽器」をコンセプトにしたシンプルな BLE MIDI ブレスコントローラーです。ワイヤレスで動作します。入手しやすいハードウェアで作れるオープンソースデバイスです。

Melody Flow でリズムを刻みながら、UniSuling で息を吹き込むことで、AI が生成したメロディに**歌うような滑らかな表現力**を加えることができます。息の強弱はリアルタイムに MIDI の**チャンネルアフタータッチ**信号に変換され、対応するシンセサイザーの音量や音色を繊細にコントロールします。

キーボードの打鍵だけでは表現が難しかった、**管楽器や声のような有機的なニュアンス**をあなたのメロディに加え、AIとのセッションをさらに深化させましょう！


<img src="docs/UniSuling_overview.jpg" alt="UniSuling Overview" width="400">
